...
float bt_vec3_length(struct bt_vec3 x) {
  return SDL_sqrtf(x.arr[0] + x.arr[1] + x.arr[2]);
}
...
void bt_mat4_mul_vec4(struct bt_mat4 const in1[static 1],
  ...
  for (size_t i = 0; i < 4; ++i) {
    *out = bt_vec4_add_vec4(
        *out, bt_vec4_mul_vec4(in1_cols[i], bt_vec4_splat(in2->arr[i])));
  }
}
